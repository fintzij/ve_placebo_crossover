...
cross = sim_settings[simnum, "cross"]
ve = sim_settings[simnum, "ve"]
frailty_sd = sim_settings[simnum, "frailty_sd"]
event_rate = sim_settings[simnum, "event_rate"]

# set up parameters
if(cross == "events") {
  n_cross = 150; t_cross = NULL
} else if(cross == "time") {
  n_cross = NULL; t_cross = 1
} else {
  n_cross = NULL; t_cross = Inf
}

# parameters - 100/6mo or decreasing from 200/6mo by 25 per 3mo
h = function(r) (pexp(0.5, r) * 1.5e3 - 50)^2
optimize(h, interval = c(0, 1))

lambda_0 = ifelse(event_rate == 300, 0.211, 0.068)

sim_pars = 
  c(VE_max = ifelse(ve == "constant", log(1 - 0.75), log(1 - 0.85)),
      VE_decay = ifelse(ve == "constant", 0, 0.977558),
      lambda_1 = lambda_0, 
      lambda_2 = lambda_0, 
      lambda_3 = lambda_0, 
      lambda_4 = lambda_0, 
...
c_haz_int = 
  function(lambda_t, t_l, t_r, trt,  
           VE_max = NULL, VE_decay = NULL, t_trt = NULL, frailty = NULL) {
...
sim_one <- function(n,
                    ...
                    n_cross,
                    t_cross,
                    enrolldur = 0.25, # enrollment period
                    crossdur = 4/52,  # crossover period
                    pvacc = 0.5,
                    frailty_pars = gamma_pars) {
...
#   for(s in seq_along(t)) {
# 
#     if(t[s] <= betas[["t0"]] | t[s] <= x[["entry"]] | t[s] > 2.25) {
#       haz_t[s] = 0
# 
#     } else {
# 
...
#                 entry = 0.1,
#                 crossover = c(0.1, 0.6),
#                 times = c(seq(0.05, 2.15, by = 0.2), 2.25, 2.25 + 1e-9)),
#     c_haz_fn = 0,
#     c_haz_man = 0)
# 
...
trials = 
  foreach(i = seq_len(iters),
          ...
          .packages = c("simsurv", "survival")) %dorng% {
            ...
            ### fit models
            # overall VE at crossover
            if(t_cross != Inf) {
              
              # generate censored dataset at crossover
              sim_cens <- cens_cross(dat = sim, 
                                     n_cross = n_cross, 
                                     t_cross = t_cross)
              
              sim_cens$tvacc = ifelse(sim_cens$trt == 1, sim_cens$entry, Inf)
              
              # fit crossover models
              fit_cross_const <- 
                coxph(formula = 
                        Surv(time = entry, 
                             time2 = eventtime, 
            ...
            par_ests = 
              data.frame(
                ...
                truth = 
                  if(ve == "constant") {
                    c(log(0.25), log(0.25), 0, log(0.25), 0)
                  } else {
                    c(log(0.15), log(0.15), 0.977558, log(0.15), 0.977558)
                  },
            ...
            if(t_cross != Inf) {
              par_ests_cross = 
                data.frame(
                  ...
                  model = c("const", "linear", "linear", "pspline", "pspline"),
                  est = c(coef(fit_cross_const), coef(fit_cross_lin), 
                          summary(fit_cross_ps)$coefficients[1:2,"coef"]),
                  var = c(vcov(fit_cross_const), diag(vcov(fit_cross_lin)),
                          summary(fit_cross_ps)$coefficients[1:2,"se(coef)"]^2),
                  row.names = NULL)
            } else {
              par_ests_cross = NULL
            }
              ...
              data.frame(
                ...
                truth = 
                  if(ve == "constant") {
                    ...
                    sim_pars["VE_max"] +  
                      c(0, 0.25, 0.5, 1, 1.5, 2) * sim_pars["VE_decay"]
                  },
                est = 0, 
                var = 0, 
                row.names = NULL)
              ...
              data.frame(
                ...
                truth = 
                  if(ve == "constant") {
                    ...
                    c(0.5, 1, 1.5, 2.0) * sim_pars["VE_decay"]
                  },
                est = 0,
                var = 0,
                row.names = NULL)
            ...
            # summarize the frailty dist
            frailty_sum = 
              data.frame(
                plac = with(subset(sim, status == 0 & trt == 0), 
                            c(mean = mean(frailty), sd = sd(frailty), 
                              quantile(frailty, c(0.1, 0.25, 0.5, 0.75, 0.9)))),
                trt = with(subset(sim, status == 0 & trt == 1), 
                           c(mean = mean(frailty), sd = sd(frailty), 
                             quantile(frailty, c(0.1, 0.25, 0.5, 0.75, 0.9)))))
            
            # return VE_ests
            return(list(par_ests = par_ests, 
                        par_ests_cross = par_ests_cross,
                        VE_ests = VE_ests, 
                        VE_decays = VE_decays,
                        n_cross = n_cross,
                        t_cross = t_cross,
                        event_counts = event_counts,
                        frailty_sum = frailty_sum))
          }

saveRDS(trials, file = paste0("frailty_sim_", ve, "_", cross, "_",event_rate,"_",frailty_sd,".Rds"))
